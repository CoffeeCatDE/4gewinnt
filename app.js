...
function create() {
  ...
  this.fieldSelectorImage = this.add.image(
    markerPositionX * iconsize + offsetx,
    iconsize - 13,
    "select"
  );
...
function isRowFree(row) {
  // ROW IS FULL
  if (gameField[0][row] == 1 || gameField[0][row] == 2) {
...
async function update(time, delta) {
  if (whoHasWonTheGame == 1) {
    this.add.text(10, 0, "||| Good! You have won this game! |||", {
      fontSize: "20px",
      fill: "#0f0",
    });
  } else if (whoHasWonTheGame == 2) {
    this.add.text(10, 0, "||| Sorry, you lost this game! |||", {
      fontSize: "20px",
      fill: "#f00",
    });
  } else {
  ...
  // NEW ROUND
  if (
    (whoHasWonTheGame == playerid || whoHasWonTheGame == computerid) &&
    ...
    if (
      ...
      if (player == playerid) {
        if (isRowFree(markerPositionX)) {
          ...
          apples = this.physics.add.sprite(
            markerPositionX * iconsize + offsetx,
            0,
            "apple"
          );
      ...
      if (player == computerid) {
        ...
        fishes = this.physics.add
          .sprite(p.x * iconsize + offsetx, 0, "fish")
          .setScale(1);
        fishes.setCollideWorldBounds(true);
        fishes.setBounce(0.1);
        physicsFishes
          .create(
            p.x * iconsize + offsetx,
            p.y * iconsize + offsety,
            "selectEnemy"
          )
          .setAlpha(0);
